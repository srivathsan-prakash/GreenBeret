//Determine directional inputs
	...
	obj_gameover.y_select = down - up;
	
//Save the previous menu position to later check if we've moved
	y_temp = obj_gameover.y_pos;

//If we have an input
	if (obj_gameover.y_select != 0)
	{
		//Modify our y_pos, keeping in mind min and max values (clamp)
		obj_gameover.y_pos = clamp(obj_gameover.y_pos + obj_gameover.y_select, 0, obj_gameover.y_max);
	
		//Provide feedback if we...
		if (obj_gameover.y_pos == y_temp)
		{ //didn't move
...
if (keyboard_check_pressed(vk_enter)) 
{
	...
	switch (argument0) 
	{
		case 0: game_restart(); break; //Main Menu
